...
class JumpEnv(MujocoEnv, utils.EzPickle):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.action_space = Box(
            low=np.array([-24,-24,-3,-24,-24,-3]), high= np.array([24,24,3,24,24,3]),
    ...
    def step(self, action):
        ...
        # discrete the actions
        action = np.round(action) / np.array([4,4,5,4,4,5])
        current_wheel_qpos = [self.data.qpos[-4],self.data.qpos[-1]]
        action[2] += self.data.qpos[-4]
        action[5] += self.data.qpos[-1]
        ...
        p_jump = min(p_jump,10)/10
        p_roll = abs(action[2])+abs(action[5])
        punishment = p_jump + p_roll
        ...
        # total reward
        reward = 20*approaching_reward + self.healthy_reward - .1*punishment + .5*jump_reward - .01*ctrl_cost
