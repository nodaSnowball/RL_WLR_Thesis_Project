...
class JumpEnv(MujocoEnv, utils.EzPickle):
    ...
    def step(self, action):
        ...
        self.c_step+=1
        # self._get_obs()
        z_prev = self.curr_obs[4]
        d_before = self.get_xydistance()
        self.do_simulation(action, self.frame_skip)
        obs = self._get_obs()
        z_curr = self.curr_obs[4]
        ...
        # cost = ctrl_cost
        # current_qpos = np.concatenate((self.curr_obs[8:10], self.curr_obs[10:12]), axis=0)
        ...
        insymmetric_punishment = sum((abs(action[:2]-action[2:])/np.array([30,30]))**2)    # 10, 20
        ...
        robot_pos = self.curr_obs[1:4].copy()
        trajector_align = - sum((target_pos-robot_pos)**2)  # 10

        jump_reward = max(0,v_z)    # 10
        
        done = self.done
        if self.curr_obs[2]>-1 and self.curr_obs[2]<1 and self.curr_obs[4]<(self.height+self._healthy_z_range):
            done = True
    ...
    def _get_obs(self):
        ...
        sensor: length = 79
            ...
            19-20  |left hip/knee qvel
            21-22  |right hip/knee qvel
            23-78  |lidar readings
            79-82  |actions
        '''
        # sensor data
        ...
        del self.curr_obs
        self.curr_obs = np.concatenate([self.target, qpos, qvel, lidar, self.action]).astype(np.float32)
    ...
    def reset_model(self):
        ...
        # self.obs = np.zeros((100, self.obs_size))
        qpos = self.init_qpos
        qvel = self.init_qvel
        # reset target
        self.height = 0.05 if self.fixed_height else random.random()*0.08+0.02
