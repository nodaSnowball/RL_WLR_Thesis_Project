...
class JumpEnv(MujocoEnv, utils.EzPickle):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.obs_size = self.data.qpos.size-2 + self.data.qvel.size-2 + 6 + self.data.sensordata.size
    ...
    def step(self, action):
        ...
        # total reward
        reward = 20*approaching_reward + self.healthy_reward - .02*punishment + .5*jump_reward - .03*ctrl_cost
    ...
    def reset_model(self):
        ...
        if self.random_reset:
            ...
            # reset obstacle 
            if action_idx<2:
                quat = Rotation.from_euler('zyx',[0, 0, np.pi/2]).as_quat()
                self.data.mocap_quat = quat
            self.data.mocap_pos[0:2] = (qpos[0:2]+qpos[2:4])/2
                
            self.set_state(qpos, qvel)
            obs = self._get_obs()
            return obs
            
        self.data.mocap_pos[0:2] = np.array([0,0])
        self.target = np.array([1.5,0])
        qpos[0:2] = self.target
        qpos[2:4] = np.array([-1.5,0])
        self.set_state(qpos, qvel)
        obs = self._get_obs()
        return obs
