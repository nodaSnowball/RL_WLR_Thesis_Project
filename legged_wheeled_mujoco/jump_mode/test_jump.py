...
def make_env(n=2000, render_mode=None):
        def _init():
                env = gym.make(args.env_name, max_step=n, healthy_reward=.3, fixed_height=0, render_mode=render_mode)
                return env
        return _init
...
# Agent
model = SAC.load(os.path.join(os.path.dirname(__file__), 'test_model/jss5e4'))
# model = SAC.load(os.path.join(os.path.dirname(__file__), '../models/jump/land'))
# evaluate_policy(model, eval_env, 10, deterministic=1, render=True, return_episode_rewards=True)
